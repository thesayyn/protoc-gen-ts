 ...
 * source: test/generation/complex.proto
 * target: node
...
export namespace event {
    ...
    export class SchedulingContext extends pb_1.Message {
        ...
        static fromObject(data: {
            ...
            if (data.env != null) {
                message.env = data.env.map(item => SchedulingContext.Env.fromObject(item));
            }
            if (data.timeout != null) {
                message.timeout = data.timeout;
            }
            if (data.batch != null) {
                message.batch = SchedulingContext.Batch.fromObject(data.batch);
            }
            return message;
        }
        toObject() {
    ...
    export namespace SchedulingContext {
        export class Env extends pb_1.Message {
            ...
            static deserialize(bytes: Uint8Array | pb_1.BinaryReader): Env {
                const reader = bytes instanceof pb_1.BinaryReader ? bytes : new pb_1.BinaryReader(bytes), message = new Env();
                while (reader.nextField()) {
                    if (reader.isEndGroup())
                        break;
                    switch (reader.getFieldNumber()) {
    ...
    export abstract class UnimplementedQueueService {
        ...
        abstract complete(call: grpc_1.ServerUnaryCall<Complete, Complete.Result>, callback: grpc_1.sendUnaryData<Complete.Result>): void;
    }
    export class QueueClient extends grpc_1.makeGenericClientConstructor(UnimplementedQueueService.definition, "Queue", {}) {
        ...
        complete: GrpcUnaryServiceInterface<Complete, Complete.Result> = (message: Complete, metadata: grpc_1.Metadata | grpc_1.CallOptions | grpc_1.requestCallback<Complete.Result>, options?: grpc_1.CallOptions | grpc_1.requestCallback<Complete.Result>, callback?: grpc_1.requestCallback<Complete.Result>): grpc_1.ClientUnaryCall => {
