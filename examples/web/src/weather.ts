...
export class WeatherClient {
    ...
    private _client: web_1.GrpcWebClientBase;
    constructor(address: string, credentials?: Object, options?: web_1.GrpcWebClientBaseOptions) {
        if (!options)
            options = {}
        options.format = options.format || "text"
        this._address = address;
        this._client = new web_1.GrpcWebClientBase(options);
    }
    private static cities = new web_1.MethodDescriptor<CityQuery, CityQuery.Result>("/Weather/cities", web_1.MethodType.UNARY, CityQuery, CityQuery.Result, (message: CityQuery) => message.serialize(), CityQuery.Result.deserialize);
    cities(message: CityQuery, metadata: web_1.Metadata | null, callback: (error: web_1.RpcError, response: CityQuery.Result) => void) {
        return this._client.rpcCall(this._address + "/Weather/cities", message, metadata || {}, WeatherClient.cities, callback);
    }
    private static get = new web_1.MethodDescriptor<GetTemperature, Temperature>("/Weather/get", web_1.MethodType.SERVER_STREAMING, GetTemperature, Temperature, (message: GetTemperature) => message.serialize(), Temperature.deserialize);
    get(message: GetTemperature, metadata: web_1.Metadata | null) {
        return this._client.serverStreaming(this._address + "/Weather/get", message, metadata || {}, WeatherClient.get);
    }
}
